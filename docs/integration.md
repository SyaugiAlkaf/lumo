...
Amanah is an on-device treasury agent: an untrusted LLM reads invoices, a
...
Configuration is one flat table read from `amanah.toml` (path in
`AMANAH_CONFIG`), overridable per key by `AMANAH_<KEY>` environment variables.
...
- Pass a `Config` to control everything: `AmanahClient(Config.profile("strict",
  db_path="treasury.db"))`.
...
`python -m amanah.api` serves a small REST API (default `127.0.0.1:8788`,
...
`python -m amanah.mcp` is a Model Context Protocol server over stdio. Register
...
|---|---|---|
| `amanah.propose_payment` | `invoice_text` | decision JSON |
| `amanah.get_status` | `intent_id` | intent row |
| `amanah.attest` | `intent_id`, `kind` (`Shipped`/`Failed`) | confirmation |
...
(`amanah/chain/adapter.py`): `create_intent`, `attest`, `release`, `refund`,
...
client = AmanahClient(Config.profile("strict", db_path="treasury.db"))
```
