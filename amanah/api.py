...
from amanah.monitor.webhooks import HttpSink, WebhookDispatcher
...
OPENAPI = {
    ...
    "info": {
        "title": "Amanah API",
        "version": "0.1.0",
        "description": "Propose invoice payments through the Amanah guard chain.",
    },
    "paths": {
        "/v1/intents": {
            "post": {
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    client: AmanahClient | None = None
    dispatcher: WebhookDispatcher | None = None
    sink = None

    def _client(self) -> AmanahClient:
        cls = type(self)
        if cls.client is None:
            cls.client = AmanahClient(cls.config)
        return cls.client
...
def serve(config: Config | None = None):
    ...
    print(f"amanah api on http://{config.api_host}:{config.api_port} (db={config.db_path})")
