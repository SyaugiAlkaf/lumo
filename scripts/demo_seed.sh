...
SUPPLIER_ADDR=$(ensure_key lumo-supplier)
ORACLE_ADDR=$(ensure_key lumo-oracle)
echo "sme      $SME_ADDR"
...
stellar tx new change-trust --line "USDC:$ISSUER_ADDR" \
    --source-account lumo-sme --network "$NETWORK" >/dev/null 2>&1 || true
stellar tx new change-trust --line "USDC:$ISSUER_ADDR" \
    --source-account lumo-supplier --network "$NETWORK" >/dev/null 2>&1 || true

echo "-- mint 10,000 USDC to sme --"
stellar contract invoke --id "$USDC_SAC" \
    --source-account lumo-issuer --network "$NETWORK" \
...
export LUMO_NETWORK="$NETWORK"
export LUMO_SME_SOURCE="lumo-sme"
export LUMO_ORACLE_SOURCE="lumo-oracle"
export LUMO_ORACLE_ADDRESS="$ORACLE_ADDR"
export LUMO_USDC_SAC="$USDC_SAC"
export LUMO_SME_ADDRESS="$SME_ADDR"
export LUMO_SUPPLIER_ADDRESS="$SUPPLIER_ADDR"
EOF
